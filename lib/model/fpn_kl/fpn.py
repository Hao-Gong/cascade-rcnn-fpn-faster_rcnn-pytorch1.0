...
from model.utils.net_utils import _smooth_l1_loss,_kl_softer_nms_loss, _crop_pool_layer, _affine_grid_gen, _affine_theta
import time
import pdb
...
class _FPN(nn.Module):
    ...
    def _init_weights(self):
        ...
        normal_init(self.RCNN_bbox_std, 0, 0.0001, cfg.TRAIN.TRUNCATED)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        bbox_std_pred=self.RCNN_bbox_std(pooled_feat)
        ...
        if self.training and not self.class_agnostic:
            ...
            # print("bbox_pred_select shape:",bbox_pred_select.shape)
            bbox_pred = bbox_pred_select.squeeze(1)
            # print("bbox_pred shape:",bbox_pred.shape)

            # print("bbox_std_pred shape:",bbox_std_pred.shape)
            bbox_std_pred_view = bbox_std_pred.view(bbox_std_pred.size(0), int(bbox_std_pred.size(1) / 4), 4)
            # print("bbox_std_pred_view shape:",bbox_std_pred_view.shape)
            bbox_std_pred_select = torch.gather(bbox_std_pred_view, 1, rois_label.long().view(rois_label.size(0), 1, 1).expand(rois_label.size(0), 1, 4))
            # print("bbox_std_pred_select shape:",bbox_std_pred_select.shape)
            bbox_std_pred = bbox_std_pred_select.squeeze(1)
            # print("bbox_std_pred shape:",bbox_std_pred.shape)
        ...
        if self.training:
            ...
            RCNN_loss_bbox = _kl_softer_nms_loss(bbox_pred,bbox_std_pred, rois_target, rois_inside_ws, rois_outside_ws)
